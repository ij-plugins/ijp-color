...
trait ColorChart {
  ...
  /** Compute average color within aligned chips from an RGB image.
    ...
    * @param cp RGB image
    */
  def averageChipColor(margin: Double, cp: ColorProcessor): Array[Array[Double]] =
    averageChipColor(margin, IJTools.splitRGB(cp))
  ...
  /** Compute average color within aligned chips from an an image.
    ...
    */
  def averageChipColor[T <: ImageProcessor](margin: Double, src: Array[T]): Array[Array[Double]]
  ...
  def referenceColorSRGB: Array[Array[Double]] = {
    referenceColorXYZ.map(xyz => colorConverter.xyz2RGB(xyz(0), xyz(1), xyz(2)).toArray)
  }
}
