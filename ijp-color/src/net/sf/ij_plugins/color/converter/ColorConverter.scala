...
final class ColorConverter(val refWhite: ReferenceWhite = ReferenceWhite.D65,
  ...
  def lab2XYZ(l: Double, a: Double, b: Double): XYZ = {
    val fy = (l + 16.0) / 116.0
    val fx = 0.002 * a + fy
    val fz = fy - 0.005 * b
  ...
  /** Convert color from CIE L*a*b* to CIE XYZ color space. */
  @inline
  def lab2XYZ(lab: Lab): XYZ = lab2XYZ(lab.l, lab.a, lab.b)

  /** Convert RGB to CIE XYZ color space. */
  def rgb2XYZ(r: Double, g: Double, b: Double): XYZ = {
  ...
  @inline
  def xyz2Lab(xyz: XYZ): Lab = xyz2Lab(xyz.x, xyz.y, xyz.z)

  /** Convert CIE XYZ to CIE L*a*b* color space. */
  def xyz2Lab(x: Double, y: Double, z: Double): Lab = {
  ...
  @inline
  def xyz2RGB(xyz: XYZ): RGB = xyz2RGB(xyz.x, xyz.y, xyz.z)

  /** Convert CIE XYZ to an RGB color space. */
  def xyz2RGB(x: Double, y: Double, z: Double): RGB = {
