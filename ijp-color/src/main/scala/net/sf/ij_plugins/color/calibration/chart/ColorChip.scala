...
object ColorChip {
  /** Create a color chip located on a [[net.sf.ij_plugins.color.calibration.chart.ChartGrid]].
    *
    ...
    * @see [[net.sf.ij_plugins.color.calibration.chart.ChartGrid]]
    * */
  def apply(name: String, color: Lab, column: Int, row: Int, margin: Double = 0): ColorChip = {
    ...
    new ColorChip(name, color,
      List(
        new Point2D(column + margin, row + margin),
        new Point2D(column + 1 - margin, row + margin),
        new Point2D(column + 1 - margin, row + 1 - margin),
        new Point2D(column + margin, row + 1 - margin)
      )
    )
  }
}
